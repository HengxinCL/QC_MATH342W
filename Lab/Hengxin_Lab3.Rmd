...
the size of data frame is 928*2, we could see some statistic informations for this data frame.
...
```{r}
ols_mod = my_simple_ols(Galton$parent,Galton$child)
```

Interpret all four quantities: b_0, b_1, RMSE and R^2. Use the correct units of these metrics in your answer.

b_0 is 23.94 This means when child's height is 0, the parent's height is 23.94
b_1 is 0.64 This means when child increased 1 centimeter, then the parent's height will be increasing 0.64 centimeters
RMSE is 2.24 This means 2.24 is the square root of the average of squared differences between prediction and actual observation
R^2 is 0.21 This means that 21% of the data fit the model.

How good is this model? How well does it predict? Discuss.

This model is not good enough, because R^2 is small.
...
ggplot(Galton, aes(x = parent, y = child)) + 
  geom_point() + 
  geom_jitter() +
  geom_abline(intercept = ols_mod$b_0, slope = ols_mod$b_1, color = "blue", size = 1) +
...
We can predict y in every x value.
